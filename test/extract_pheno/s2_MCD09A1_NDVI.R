...
satellite = "MCD09A1"
df_Terra = tidy_modis_09A1(satellite = "MOD09A1")
df_Aqua  = tidy_modis_09A1(satellite = "MYD09A1")

df = list(Terra = df_Terra, Aqua = df_Aqua) %>% melt_list("sate")
setkeyv(df, c("site", "group", "t"))
...
lst_NDVI = foreach(i = grps_sites, icount()) %dopar% {
    ...
    l_site = foreach(j = grps, icount()) %do% {
        ...
        lambda <- tryCatch({
            l_lambda = v_curve(d[!is.na(y)], lg_lambdas = seq(2, 3.7, 0.05), IsPlot = FALSE)
            ans = l_lambda$lambda
            if (ans > 2000) ans <- ans*2/3
            ans
...
{
    version = 0.4
    outfile = glue("./Figures_phenofit/phenofit_{satellite}_NDVI_{version}.pdf")
    merge_pdf(outfile, pattern = "\\[", del = TRUE)
    pdf_acrobat(outfile)
}
save(lst_NDVI, file = glue("INPUT/pheno_{satellite}_NDVI_st166.rda"))
