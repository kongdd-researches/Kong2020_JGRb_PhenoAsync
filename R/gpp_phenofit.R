...
#' @export
main_phenofit <- function(l, TRS = c(0.1, 0.2, 0.5, 0.6, 0.8),
                          show = TRUE, first.fig = TRUE,
                          verbose = TRUE)
{
  ...
  if (show) {
    g <- plot_phenofit(d_fit, l$brks, l$titlestr, title.ylab = "NDVI", "Time",
                       shape = "point", cex = 0.4,
                       theme = NULL)
    ...
    outfile = "phenofit_multi_seasons_v4.pdf")
{
  ...
  lst = foreach(sitename = sites, i = icount()) %do% {
    ...
    tryCatch({
      l = divide_seasons(d, sp, 365,
                         ...
                         .check_season = FALSE)
      l$titlestr = with(sp[1, ], sprintf("%s, %s, [%.2f, %.2f] lambda = %.1f", 
        site, IGBP, lon, lat, l$lambda)) # %dth, ID
      l
  ...
  for (i in grps) {
    ...
    if (calendarYear) {
      ...
      lst[[i]]$dt <- tidy_seasons(l, rtrough_max = rtrough_max, r_min = 0.1, ypeak_min = 5, show = show)
    }
  }

  if (show) { dev.off(); SumatraPDF(outfile) }
  lst
}
