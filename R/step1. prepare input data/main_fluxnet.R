...
# 	Rcpp mean_cpp and sum_cpp function
# Rcpp::sourceCpp('MeanAndSum.cpp') #MEAN AND SUM function in Rcpp
Rcpp::sourceCpp('MeanAndSum.cpp')
...
tidy_daytime <- function(dt){
    ...
    prcp <- dt[, .(P_F = sum_cpp(P_F)), by = date]
    ...
    x_daily   <- dt_dlight[, lapply(.SD, mean_cpp), by = date,
    ...
    # 5. Convert Carbon flux units (from umol/m2/s to g/cm2/d)
